 ...
 * 
 */
public class QueueableCachedThreadPool extends java.util.concurrent.ThreadPoolExecutor {
  ...
  public QueueableCachedThreadPool(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit,
   ...
   * or in the calling thread, at the discretion of the <tt>Executor</tt> implementation. If no threads are available,
   * it will be added to the work queue. If the work queue is full, the system will wait for the specified time and it
   * throw a RejectedExecutionException if the queue is still full after that.
   *
  ...
  public static class ControllableQueue extends LinkedBlockingQueue<Runnable> {
    ...
    private volatile QueueableCachedThreadPool parent = null;

    public ControllableQueue(int capacity) {
      super(capacity);
    }

    public void setParent(QueueableCachedThreadPool tp) {
      parent = tp;
    }
